  ...
  try {
    ...
    let allMatches = matchesData.map((m: any) => {
      ...
      return {
        ...
        id: m.id,
        status: m.status,
        scheduled_at: m.scheduledAt || m.scheduled_at,
        scheduled_time: m.scheduledTime || m.scheduled_time,
        matchday: m.matchday,
        matchday_number: m.matchday || m.matchday_number,
        round: m.round,
        stage: m.stage,
        round_name: m.roundName || m.round_name,
        group_name: m.groupName || m.group_name,
        score_home: m.scoreHome ?? m.score_home ?? null,
        score_away: m.scoreAway ?? m.score_away ?? null,
        reported_score_home: m.reportedScoreHome ?? m.reported_score_home ?? null,
        reported_score_away: m.reportedScoreAway ?? m.reported_score_away ?? null,
        winner_team_id: m.winnerTeamId || m.winner_team_id || null,
        next_match_id: m.nextMatchId || m.next_match_id || null,
        next_match_slot: m.nextMatchSlot || m.next_match_slot || null,
        home_team_id: m.homeTeamId || m.teamHomeId || m.home_team_id || m.team_home_id,
        away_team_id: m.awayTeamId || m.teamAwayId || m.away_team_id || m.team_away_id,
        team_home_id: m.homeTeamId || m.teamHomeId || m.home_team_id || m.team_home_id,
        team_away_id: m.awayTeamId || m.teamAwayId || m.away_team_id || m.team_away_id,
        home_team_name,
        home_team_tag,
        home_team_logo_url,
        away_team_name,
        ...
        competition_id: m.competitionId || m.competition_id || comp?.id,
        tournament_id: m.tournamentId || m.tournament_id || comp?.id,
