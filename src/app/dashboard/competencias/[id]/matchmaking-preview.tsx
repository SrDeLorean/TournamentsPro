...
function LeagueStandingsMock({ teams }: { teams: TeamItem[] }) {
  return (
    <Card className="glass-panel p-5 space-y-4 shadow-xl border-[var(--app-accent-2)]/30">
      <div className="flex items-center justify-between border-b border-[var(--text-heading)]/10 pb-3">
        <div className="flex items-center gap-2 font-black text-xs uppercase text-[var(--app-accent-2)]">
          <Trophy className="w-4 h-4 text-[var(--app-accent-2)]" />
          <span>Esqueleto de Tabla de Posiciones Oficial ({teams.length} Clubes)</span>
        </div>
        <Badge className="bg-[var(--app-accent-2-soft)] text-[var(--app-accent-2)] font-[family-name:var(--font-active)] text-[10px]">
        ...
        <table className="w-full text-left text-xs font-[family-name:var(--font-active)]">
          <thead>
            <tr className="border-b border-[var(--text-heading)]/10 text-[var(--text-muted)] text-[10px] uppercase">
              ...
              <th className="p-2 text-center text-[var(--app-accent)] font-bold">PTS</th>
            </tr>
          </thead>
          <tbody>
            {teams.slice(0, 16).map((t, idx) => (
              <tr
                key={t.id}
                className={`border-b border-[var(--text-heading)]/5 hover:bg-[var(--app-surface-2)]/40 transition-colors ${
                  idx < 4 ? 'bg-[var(--app-positive-soft)]/20' : ''
                }`}
              >
                <td className="p-2 font-black text-[var(--text-muted)]">
                  <span
                    className={`w-5 h-5 inline-flex items-center justify-center rounded text-[10px] ${
                      idx === 0
                        ? 'bg-[var(--app-warning)] text-[var(--accent-contrast)] font-black'
                        : idx < 4
                ...
                <td className="p-2 font-black text-[var(--text-heading)] flex items-center gap-2">
                  <div className="w-6 h-6 rounded bg-[var(--app-surface-2)] border border-[var(--app-accent-2)]/40 flex items-center justify-center text-[9px] text-[var(--app-accent-2)] font-black">
                    {(t.tag || t.name.slice(0, 3)).toUpperCase()}
                  </div>
                  <span>{t.name}</span>
                </td>
                <td className="p-2 text-center text-[var(--text-muted)]">0</td>
                <td className="p-2 text-center text-[var(--text-muted)]">0</td>
                <td className="p-2 text-center text-[var(--text-muted)]">0</td>
                <td className="p-2 text-center text-[var(--text-muted)]">0</td>
                <td className="p-2 text-center text-[var(--text-muted)]">0</td>
                <td className="p-2 text-center text-[var(--text-muted)]">0</td>
                <td className="p-2 text-center text-[var(--text-muted)]">0</td>
                <td className="p-2 text-center font-black text-[var(--app-accent)]">0</td>
              </tr>
            ))}
          </tbody>
        </table>
...
function PaginatedMatchdayScheduleTable({
  ...
  return (
    <Card className="glass-panel p-5 space-y-4 shadow-xl border-[var(--text-heading)]/10">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-[var(--text-heading)]/10 pb-3">
          ...
          <tbody>
            {currentMatches.map((m) => (
              <tr key={m.id} className="border-b border-[var(--text-heading)]/5 hover:bg-[var(--app-surface-2)]/40 transition-colors">
                <td className="p-2">
                  <span className="px-2.5 py-1 rounded bg-[var(--app-accent-soft)] text-[var(--app-accent)] border border-[var(--app-accent)]/40 font-[family-name:var(--font-active)] font-bold text-xs inline-flex items-center gap-1.5">
                    <Calendar className="w-3.5 h-3.5 text-[var(--app-accent)]" />
                    <span>{m.scheduledDateLabel}</span>
                  </span>
                </td>
                <td className="p-2 text-right font-black text-[var(--text-heading)]">{m.homeTeamName}</td>
                <td className="p-2 text-center font-black text-[var(--text-muted)] text-[10px]">VS</td>
                <td className="p-2 text-left font-black text-[var(--text-heading)]">{m.awayTeamName}</td>
                <td className="p-2 text-center">
                  <span className="px-2.5 py-1 rounded bg-[var(--app-surface-2)] text-[var(--app-accent-2)] border border-[var(--app-accent-2)]/30 text-xs font-[family-name:var(--font-active)] font-bold">
      ...
      <div className="flex items-center justify-between border-t border-[var(--text-heading)]/10 pt-3 flex-wrap gap-2">
        <Button
          variant="ghost"
          size="sm"
          disabled={currentMatchday <= 1}
          onClick={() => setCurrentMatchday((prev) => Math.max(1, prev - 1))}
          className="glass-panel-hover text-xs font-bold text-[var(--text-secondary)] hover:text-[var(--text-heading)] disabled:opacity-30"
        >
        ...
        <div className="flex items-center gap-1 overflow-x-auto max-w-[350px] px-2 py-1">
          {matchdayNumbers.map((jNum) => (
            <button
              ...
              className={`w-7 h-7 flex-shrink-0 rounded-lg text-xs font-[family-name:var(--font-active)] font-bold transition-all ${
                currentMatchday === jNum
                  ? 'bg-[var(--app-accent)] text-[var(--accent-contrast)] shadow-md font-black scale-105'
                  : 'glass-panel-hover text-[var(--text-muted)] hover:text-[var(--text-heading)]'
              }`}
            >
              {jNum}
            </button>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Badge className="bg-[var(--app-positive-soft)] text-[var(--app-positive)] border-[var(--app-positive)]/40 text-[10px] font-[family-name:var(--font-active)]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {groups.map((group) => (
          <Card key={group.groupName} className="glass-panel p-4 space-y-3 border-[var(--text-heading)]/10 shadow-lg">
            <div className="flex items-center justify-between border-b border-[var(--text-heading)]/10 pb-2">
              <span className="text-xs font-black uppercase text-[var(--app-accent)]">{group.groupName}</span>
              <span className="text-[10px] font-[family-name:var(--font-active)] text-[var(--text-muted)]">{group.count} Equipos</span>
            </div>

            <table className="w-full text-left text-[11px] font-[family-name:var(--font-active)]">
              <thead>
                <tr className="border-b border-[var(--text-heading)]/10 text-[var(--text-muted)] uppercase text-[9px]">
                  <th className="p-1">Pos</th>
                  <th className="p-1">Club</th>
                  <th className="p-1 text-center">PJ</th>
                  <th className="p-1 text-center font-bold text-[var(--app-accent)]">PTS</th>
                </tr>
              </thead>
              <tbody>
                {group.teams.map((t, idx) => (
                  <tr
                    key={t.id}
                    className={`border-b border-[var(--text-heading)]/5 ${
                      idx < 2 ? 'bg-[var(--app-positive-soft)]/30 text-[var(--app-positive)]' : 'text-[var(--text-secondary)]'
                    }`}
                  >
                    <td className="p-1 font-bold">
                      <span className={`px-1.5 py-0.5 rounded text-[9px] ${idx < 2 ? 'bg-[var(--app-positive)]/20 text-[var(--app-positive)] font-bold' : 'text-[var(--text-muted)]'}`}>
                        #{idx + 1}
                      </span>
                    </td>
                    <td className="p-1 font-bold truncate max-w-[120px]">{t.name}</td>
                    <td className="p-1 text-center text-[var(--text-muted)]">0</td>
                    <td className="p-1 text-center font-black text-[var(--app-accent)]">0</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
function BracketViewer({
  ...
  return (
    <Card className="glass-panel p-6 space-y-5 border-[var(--app-accent)]/30 shadow-2xl overflow-x-auto">
      <div className="flex items-center justify-between border-b border-[var(--text-heading)]/10 pb-3">
        <div className="flex items-center gap-2 font-black text-xs uppercase text-[var(--app-accent)]">
        ...
        <Badge className="bg-[var(--app-accent-soft)] text-[var(--app-accent)] font-[family-name:var(--font-active)] text-[10px]">
        ...
        {Object.entries(roundMap).map(([roundOrderStr, roundNodes]) => {
          ...
          return (
            <div key={rOrder} className="flex-1 flex flex-col justify-around space-y-4">
              <div className="text-center p-2 rounded-xl bg-[var(--app-surface-2)]/90 border border-[var(--app-accent-2)]/30">
                <span className="text-xs font-black uppercase text-[var(--app-accent-2)] block">
                  {roundName}
                </span>
                <span className="text-[9px] font-[family-name:var(--font-active)] text-[var(--app-accent)] font-bold block mt-0.5">
              ...
              <div className="space-y-4 flex flex-col justify-around flex-1">
                {roundNodes.map((match) => (
                  <div
                    key={match.id}
                    className="p-3 rounded-xl bg-[var(--app-canvas)]/90 border border-[var(--text-heading)]/15 hover:border-[var(--app-accent)]/50 transition-all shadow-md space-y-2 relative group"
                  >
                    <div className="flex items-center justify-between text-[9px] font-[family-name:var(--font-active)] text-[var(--text-muted)] border-b border-[var(--text-heading)]/10 pb-1">
                      <span>ID: {match.id.slice(-8)}</span>
                      <span className="text-[var(--app-positive)] font-bold">Simultáneo</span>
                    </div>
                    ...
                    {match.nextMatchId && (
                      <div className="text-[9px] font-[family-name:var(--font-active)] text-[var(--app-accent-2)] pt-1 border-t border-[var(--text-heading)]/5 flex items-center justify-between">
                        <span className="flex items-center gap-1">
                          <ArrowRight className="w-3 h-3 text-[var(--app-accent)]" /> Avance a siguiente ronda
                        </span>
                        <span className="text-[8px] font-black text-[var(--text-muted)] uppercase">[{match.nextMatchSlot}]</span>
                      </div>
                    )}
                  </div>
                ))}
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-200">
      <div className="glass-panel p-5 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-2xl">
        <div>
          <div className="flex items-center gap-2">
            <Sparkles className="w-5 h-5 text-[var(--app-accent)] animate-pulse" />
            <h3 className="text-base font-black uppercase text-[var(--text-heading)] tracking-wider">
          ...
          <p className="text-xs text-[var(--text-muted)] font-[family-name:var(--font-active)] mt-1">
            Fecha Final Estimada del Torneo: <strong className="text-[var(--app-positive)]">{finalProjectedDate}</strong>
          </p>
        </div>
        ...
        {onConfirmSave && (
          <Button
            ...
            className="bg-[var(--app-positive)] hover:bg-[var(--app-positive)] text-[var(--accent-contrast)] font-black text-xs px-6 py-3 rounded-xl shadow-xl flex items-center gap-2"
          >
