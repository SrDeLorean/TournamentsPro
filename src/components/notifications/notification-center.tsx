          ...
          <div className="flex items-center justify-between border-b border-[var(--border-card)] pb-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <span className="text-[9px] font-mono text-[var(--text-muted)] font-bold">{unreadCount ? `${unreadCount} pendientes` : 'Todo al día'}</span>
              </div>
            </div>
            ...
            {unreadCount > 0 && (
              <button
                ...
                className="text-[10px] text-[var(--accent-cyan)] hover:text-[var(--text-heading)] font-extrabold flex items-center gap-1 bg-[var(--accent-cyan-bg)] px-2 py-1.5 rounded-lg border border-[var(--accent-cyan)]/30"
              >
            ...
            {([
              ...
              <button
                ...
                className={`px-2.5 py-1 rounded-lg transition-all flex-shrink-0 border ${
                  activeFilter === tab.id
                    ? 'bg-[var(--accent-cyan)] text-[var(--bg-main)] font-black border-[var(--accent-cyan)] shadow-sm'
              ...
              filteredNotifications.map((n) => (
                <div
                  ...
                  className={`notification-center-item p-3 rounded-xl border transition-all text-xs space-y-1 relative group ${
                    !n.isRead
                      ? 'is-unread border-[var(--accent-cyan)]/40'
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1">
                      <span className="text-[9px] font-mono text-[var(--text-muted)]">{n.timestamp}</span>
                      <button
                        type="button"
                        onClick={() => removeNotification(n.id)}
                        aria-label={`Eliminar notificación: ${n.title}`}
                        className="p-1 text-[var(--text-muted)] hover:text-rose-400 opacity-60 sm:opacity-0 group-hover:opacity-100 focus:opacity-100 transition-opacity"
                      >
                  ...
                  {n.actionUrl && (
                    <Link
                      ...
                      className="text-[10px] text-[var(--accent-cyan)] font-black hover:underline inline-flex items-center gap-1 pt-1"
                    >
