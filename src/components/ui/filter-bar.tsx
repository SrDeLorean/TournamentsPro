...
export function FilterBar({
  ...
  return (
    ...
    >
      ...
      <div className="relative w-full min-w-0 flex-1 lg:min-w-[220px] group">
        <Search
        ...
        <input
          ...
          className="ui-control w-full h-11 pl-11 pr-10 text-xs sm:text-sm font-mono placeholder:text-[var(--text-muted)]"
        />
        {searchValue && (
          <button
            type="button"
      ...
      {options.length > 0 && onFilterChange && (
        renderAsSelect ? (
          ...
          <div className="mobile-scroll-row flex w-full lg:w-auto items-center gap-2 overflow-x-auto pb-1 lg:pb-0 touch-pan-x flex-shrink-0">
            {options.map((opt) => {
              ...
              return (
                <button
                  ...
                  className={`px-3.5 py-2 rounded-[var(--ui-radius-control)] text-xs font-mono font-bold transition-all duration-200 flex items-center gap-1.5 whitespace-nowrap border select-none active:scale-95 ${
                    isActive
                      ? 'shadow-md'
      ...
      {typeof count === 'number' && (
        <div className="text-[10px] font-mono font-bold text-[var(--text-muted)] uppercase tracking-widest px-3.5 py-2 bg-[var(--bg-subtle)] rounded-[var(--ui-radius-control)] border border-[var(--border-card)] shrink-0 flex items-center gap-1.5 self-start lg:self-auto shadow-sm">
